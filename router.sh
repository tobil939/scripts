...
# License: MIT
# Description: Installation script for setting up my router, reworked with grok
# Version: v8.0
# Variables
...
checkinstall() {
  ...
  arch=false
  rocky=false
  command -v pacman >/dev/null && horst="1"
  command -v dnf >/dev/null && horst="2"
  if [[ "$horst" == 0 ]]; then
    log "not checked for packages manager"
  elif [[ "$horst" == 1 ]]; then
    arch=true
    log "OS is Arch"
  elif [[ "$horst" == 2 ]]; then
...
directory mask = 0775
EOF
log "Setting up Btrfs subvolumes & mounts"
  # Erst Verzeichnisse + Subvolumes anlegen
  ...
  # fstab nur ergänzen, wenn nicht vorhanden
  #echo "UUID=$UUID1 /router1 btrfs subvol=router1,defaults 0 2" | sudo tee -a /etc/fstab
  #echo "UUID=$UUID2 /router2 btrfs subvol=router2,defaults 0 2" | sudo tee -a /etc/fstab
  #sudo systemctl daemon-reload
  #sudo mount -a || { log "mount -a fehlgeschlagen – siehe dmesg"; exit 1; }
