...
drive1="sda"
drive2="nvme0n1"
UUID1=$(sudo blkid /dev/${drive1}2 | grep -oP 'UUID="\K[^"]+')
UUID2=$(sudo blkid /dev/${drive2}p2 | grep -oP 'UUID="\K[^"]+')
WAN_IF="lo"
LAN_ETH="enp1s0"
LAN_WLAN="wlan0"
...
deactivated() {
  ...
  sudo systemctl is-active --quiet daemon-reload && systemctl stop daemon-reload || true
  log "Stop hostapd"
...
bashing() {
  ...
  cat >>"/home/tobirouter/.bashrc" <<'HERE'
# ~/.bashrc
#
